...
export async function GET(request: Request) {
  try {
    ...
    const total_results = await Post.countDocuments({ userID: user._id }).where("type").ne("post");
    const total_pages = Math.ceil(total_results / limit);
    const posts = await Post.find({ userID: user._id }).where("type").ne("post").populate({path: "userID",select: "userName name avatar"})
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
