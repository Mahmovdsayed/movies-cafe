...
export async function GET(request: Request) {
  try {
    ...
    const total_results = await Post.countDocuments({ userID: user._id })
      .where("type")
      .ne("repost");
    const total_pages = Math.ceil(total_results / limit);
    const reposts = await Post.find({ userID: user._id })
      .where("type")
      .ne("repost")
      .populate({ path: "userID", select: "userName name avatar" })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
